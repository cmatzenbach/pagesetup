...
<div id="step1">
<p><strong>Welcome to the web/ page setup - multiple content edition</strong></p><br />
<h3>1. Have you setup $multiplenavtabslist in your EventVariables file?</h3>
<p>Preview:</p>
<div id="multiplenavtabspreview">
<ul>
<?php 
	unset($regtab);
	$iii = 0;
	$subsitename = array_keys($multiplenavtabslist);
	foreach ($multiplenavtabslist as $subsite) {
	  echo "<li>" . $subsitename[$iii];
	  $iii++;
	  echo "</li><ul>";
	  foreach ($subsite as $tab) {
		$tab = explode("+",$tab);
		echo "<li>" . $tab[0];
...
<p> - Click through the menu below to preview the different content templates using your site's own css. When you like a template, click "Apply Now" to add the file to your content folder.<br />
</p>
<style>
...
<div id="dropdowns">
	<div id="templates">
		<form action="<?php echo $whereiam?>#drop" method="get" name="listing" id="listing">
			<input type="hidden" name="option" value="pagesetupmultiple">
...
$mytemplatefolders = glob($dotdotslash . '../global/templates/*', GLOB_ONLYDIR); //get all folders in global/templates
$folderlinks = $mytemplatefolders;
// Alphabetize
natcasesort($mytemplatefolders); //alphabetize template folders
$alltemplatetypes = array();
$alltemplatefiles = array();
$mytemplatefolders = str_replace($dotdotslash . '../global/templates/','',$mytemplatefolders); //remove path to isolate folder name
$mytemplatefolders = str_replace('_',' ',$mytemplatefolders);
foreach ($mytemplatefolders as $templatefolder){
	$templatefolder = ucwords($templatefolder); //make first letter of ach word in foldername uppercase
	$alltemplatetypes[] = $templatefolder; //create storage array for names of template folders
	if(!$selectedtemplate){}
	else {
...
$contentfolderoptions = $multiplenavtabslist; //make copy of multiplenavtabs so I can add generic content folder (root content) to list without messing up other uses of multiplenavtabslist above
$ee = 0;
foreach($contentfolderoptions as $navtabs) {
	$contentcounter[$ee] = count($navtabs); //counts how many content files are in each folder
	$ee++;
}

$contentbase = array(); //create array for the root content folder
foreach($contentfolderoptions as $contentfolders) { 
	foreach($contentfolders as $contentfiles) {
		$contentbase[] = $contentfiles . "-root"; //combine all elements from subsite arrays into root content array
	}
}

$contentbase = array_unique($contentbase); //remove duplicates
$contentfolderoptions["content"] = $contentbase; //stick root content folder in the multnavtabs list

if ($splashpage) { $contentbase[] = "Splash Page"; } //not sure if this works or not, stick in content root
$subsitename2 = array_keys($contentfolderoptions); //newsubsitename to include the generic content root
$ff = 0;
$gg = 2; //for some reason i have yet to figure out, the root folder names start at option[2] (maybe it's the loading message that get's taken out prior to displaying on page?)

foreach($contentfolderoptions as $navtabs) {
	echo "dropdownnavs.options[dropdownnavs.options.length] = new Option(\"" . $subsitename2[$ff] . "\",\"" . $subsitename2[$ff] . "\");";
	echo "dropdownnavs.options[" . $gg. "].disabled=true;"; //disable content folders from being options
	$key = $subsitename2[$ff]; //assigns key to build $multnavtabsclean array (for searching with recursive_array_function)
foreach($navtabs as $tab)
{	
	$tab = explode("+",$tab); //in case subtabs exist
	if ($tab[0] == "Splash Page") {  //not sure how to handle this, may not work
		$splashpagelink = str_replace(["content/",".php"], '', $splashpagelink);
		$href = $splashpagelink;
	 }
	else { //if not a splash page
	$href = strtolower(str_replace($badcharacters, '', $tab[0])); } //removes badcharacters, makes all lowercase for link
	$multnavtabsclean[$key][] = $href; //adds current file to $multnavtabsclean array, for searching later
	if ($tab[1] && $submenublank == true) {} //if submenu pages are just anchor points do nothing
	else {
		if(!$regpagephp){$regtab = substr($tab[0],0,3);} //if there's no regpagephp
			if($regtab == "Reg") {} //reg is not an actual content page
...
<?php if ($_POST['submit'])
{
	$endofname = substr($selectednavtab,-4);
	if($endofname == "root") {  //this belongs in the generic content folder?
	...
	if($endofname !== "root") { //this does not have root at end and belonds in a specific content folder, determined by $rootlocation found through recursive_array_search
		$rootlocation = recursive_array_search($selectednavtab, $multnavtabsclean);
		if(copy($folderlinks[$folderlocation] . '/' . $selectedtemplate, 'content/' . $rootlocation . '/' . $selectednavtab . ".php")) 
		{
			echo "<br><b>Copied " . $selectedtemplate . " to " . $rootlocation . "/" . $selectednavtab . " successfully! Remember to GET the files from Remote Server in Dreamweaver to be able to edit.</b><br>";
		}
	}
}?>
<div id="results2"><br /></div></div>
